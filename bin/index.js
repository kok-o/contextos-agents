...
if (mainCommand === 'detect') {
  const { WorkspaceGraphBuilder } = require('../.agents/workspace/workspace-graph.js');
  ...
  if (scopeArg) {
    ...
    console.log('    contextos detect --scope <path>    Scope detection to a specific file/subproject');
    console.log('    contextos detect --explain         Display full workspace evidence graph\n');
  }
  process.exit(0);
}
...
// Recover command
if (mainCommand === 'recover') {
  const recoverModule = require('./commands/recover.js');
  recoverModule(args, flags);
  process.exit(0);
}
