/**
 * .agents/runtime/state-machine.cjs
...
const { sha256, computeScopeSha256, computeDiffSha256, verifyAttestationSubject } = require("./attestations.cjs");
...
const LEGAL_EXECUTION_TRANSITIONS = {
	[EXECUTION_STATUS.QUEUED]: [EXECUTION_STATUS.PREPARING, EXECUTION_STATUS.CANCELLED, EXECUTION_STATUS.INTERRUPTED],
...
const LEGAL_VERIFICATION_TRANSITIONS = {
	[VERIFICATION_STATUS.NOT_CONFIGURED]: [VERIFICATION_STATUS.PENDING, VERIFICATION_STATUS.NOT_APPLICABLE],
	[VERIFICATION_STATUS.NOT_APPLICABLE]: [VERIFICATION_STATUS.PENDING],
	...
	[VERIFICATION_STATUS.FAIL]: [VERIFICATION_STATUS.PENDING, VERIFICATION_STATUS.STALE],
	[VERIFICATION_STATUS.ERROR]: [VERIFICATION_STATUS.PENDING, VERIFICATION_STATUS.STALE],
	[VERIFICATION_STATUS.TIMEOUT]: [VERIFICATION_STATUS.PENDING, VERIFICATION_STATUS.STALE],
	[VERIFICATION_STATUS.STALE]: [VERIFICATION_STATUS.PENDING, VERIFICATION_STATUS.RUNNING],
};

const LEGAL_REVIEW_TRANSITIONS = {
	[REVIEW_STATUS.NOT_CONFIGURED]: [REVIEW_STATUS.PENDING],
	[REVIEW_STATUS.PENDING]: [REVIEW_STATUS.RUNNING, REVIEW_STATUS.STALE],
	...
	[REVIEW_STATUS.PASS]: [REVIEW_STATUS.STALE, REVIEW_STATUS.PENDING],
	[REVIEW_STATUS.FAIL]: [REVIEW_STATUS.PENDING, REVIEW_STATUS.STALE],
	[REVIEW_STATUS.ERROR]: [REVIEW_STATUS.PENDING, REVIEW_STATUS.STALE],
	[REVIEW_STATUS.TIMEOUT]: [REVIEW_STATUS.PENDING, REVIEW_STATUS.STALE],
	[REVIEW_STATUS.UNAVAILABLE]: [REVIEW_STATUS.PENDING, REVIEW_STATUS.STALE],
	[REVIEW_STATUS.MALFORMED]: [REVIEW_STATUS.PENDING, REVIEW_STATUS.STALE],
	[REVIEW_STATUS.STALE]: [REVIEW_STATUS.PENDING, REVIEW_STATUS.RUNNING],
};

const LEGAL_MERGE_TRANSITIONS = {
	[MERGE_STATUS.NOT_READY]: [MERGE_STATUS.READY, MERGE_STATUS.BLOCKED, MERGE_STATUS.CONFLICT, MERGE_STATUS.STALE_BASE],
	[MERGE_STATUS.READY]: [MERGE_STATUS.MERGING, MERGE_STATUS.NOT_READY, MERGE_STATUS.BLOCKED, MERGE_STATUS.STALE_BASE],
	[MERGE_STATUS.MERGING]: [MERGE_STATUS.MERGED, MERGE_STATUS.CONFLICT, MERGE_STATUS.ERROR, MERGE_STATUS.BLOCKED],
	[MERGE_STATUS.MERGED]: [],
	[MERGE_STATUS.BLOCKED]: [MERGE_STATUS.NOT_READY, MERGE_STATUS.READY],
	[MERGE_STATUS.CONFLICT]: [MERGE_STATUS.NOT_READY],
	[MERGE_STATUS.STALE_BASE]: [MERGE_STATUS.NOT_READY],
	[MERGE_STATUS.ERROR]: [MERGE_STATUS.NOT_READY],
};
...
function evaluateMergeReadiness(thread, options = {}) {
	...
	if (revStatus !== REVIEW_STATUS.PASS) {
		...
		if (!subjCheck.valid) {
			...
			!revAtt.llmVerdict ||
			revAtt.llmVerdict.specCompliance !== "PASS" ||
			revAtt.llmVerdict.codeQuality !== "PASS"
		) {
...
function transitionThread(thread, event = {}, options = {}) {
	...
	switch (event.type) {
		...
		case "VERIFICATION_CONFIGURED": {
			_assertLegalTransition(
				"verification",
				thread.verification.status,
				VERIFICATION_STATUS.PENDING,
				LEGAL_VERIFICATION_TRANSITIONS,
			);
...
function _assertLegalTransition(facet, fromStatus, toStatus, legalTable) {
	...
	if (!allowed.includes(toStatus)) {
		const err = new Error(`Illegal ${facet} state transition: cannot transition from ${fromStatus} to ${toStatus}`);
