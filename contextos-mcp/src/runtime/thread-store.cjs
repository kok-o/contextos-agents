/**
 * .agents/runtime/thread-store.cjs
...
const { createThreadState, transitionThread } = require("./state-machine.cjs");
...
class ThreadStore {
	...
	_getThreadPath(threadId) {
		...
		return path.join(this.storeDir, `${safeId}.cjson`);
	}

	_getAuditPath(threadId) {
		const safeId = threadId.replace(/[^a-zA-Z0-9_-]/g, "_");
		return path.join(this.storeDir, `${safeId}.audit.cjsonl`);
	}
	 ...
	 * @returns {import('../../../.agents/transaction-core/ipc-lock').LeaseLock}
	 */
	 ...
	 * @param {Object} thread
	 * @param {import('../../../.agents/transaction-core/ipc-lock').LeaseLock} [lease] - Optional lease lock to enforce ownership.
	 */
	...
	list() {
		...
		const files = fs.readdirSync(this.storeDir).filter((f) => f.endsWith(".cjson") && !f.endsWith(".tmp"));
		const threads = [];
