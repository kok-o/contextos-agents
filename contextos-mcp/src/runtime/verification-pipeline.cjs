/**
 * .agents/runtime/verification-pipeline.cjs
...
const { sha256, createVerificationAttestation } = require("./attestations.cjs");
...
function validateCommandTrust(spec, options = {}) {
	...
	for (const arg of spec.args) {
		if (FORBIDDEN_EVAL_ARGS.has(arg.toLowerCase())) {
			const err = new Error(
				`Security error: Inline code evaluation argument "${arg}" is prohibited in verification spec.`,
			);
